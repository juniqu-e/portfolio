...
```ts
{
  name?: string;       // 선택, 최대 20자, 비어 있으면 "익명"
  message: string;     // 필수, 1~100자
...
const schema = z.object({
  name: z.string().max(20).optional().default("익명"),
...
CREATE TABLE guestbook (
  id          TEXT PRIMARY KEY,           -- ULID 또는 nanoid
  name        TEXT NOT NULL DEFAULT '',   -- 빈 문자열이면 "익명" 처리
  message     TEXT NOT NULL,
  ip_hash     TEXT NOT NULL,              -- IP의 SHA-256 hash (raw IP 저장 X)
