...
- [head] **in_progress** — Phase 12 (admin 인증 재설계) 병행 dispatch. backend: lib/admin-auth.ts 재작성 (Bearer → HMAC 쿠키), POST /api/admin/login + logout, middleware.ts /admin/\* redirect, 기존 admin API 4종 쿠키 검증으로, .env.example 갱신 (ADMIN_USERNAME/PASSWORD_HASH/SESSION_SECRET, ADMIN_TOKEN 제거). frontend: /admin/guestbook → /admin 단일, AdminLogin (ID+PW 폼), Bearer 헤더 제거. 머지 후 사용자 ID/PW 결정 → head bcrypt + secret 생성 → .env 갱신 → 재배포
